...
# 배경과 물체를 지정
# 배경이다(확정) cv2.GC_BGD(확정배경)
# 배경일거야(확률) cv2.GC_PR_BGD 
# 물체이다 cv2.GC_FGD(확정물체)
# 물체일거야(확률) cv2.GC_PR_FGD

# 마스크는 기본적으로 배경일거야 라는 전제를 갖고 있음
...
grad_img = img * end_mask_img[:,:,np.newaxis] # 차원을 늘려서 필요없는 영역과 필요한 영역에 곱을 하면 내가 표시한 영역만 살아남음
print(grad_img.shape)
cv2.imshow('grad_img',grad_img) # 영역 추출 결과 이미지
