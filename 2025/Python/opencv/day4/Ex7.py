...
d = load_img('data4-1.jpg', 'data4-2.jpg')
...
flann_matcher = cv2.DescriptorMatcher().create(cv2.DescriptorMatcher_FLANNBASED) # 특징 매칭
knn_matcher = flann_matcher.knnMatch(ck_des, new_des, 2)
T = 0.5 # 정리 기준
m_l = [ck_des for ck_des, new_des in knn_matcher if ck_des.distance/new_des.distance<T] # 특징 매칭 정보
print(ck_kp[52].pt)
# for i in m_l:
...
H, _ = cv2.findHomography(kp_ck, kp_new, cv2.RANSAC) # 호모그래피 추정
...
mc_img = np.empty((max(ck_img.shape[0], new_img.shape[0]), ck_img.shape[1]+new_img.shape[1],3), np.uint8)
